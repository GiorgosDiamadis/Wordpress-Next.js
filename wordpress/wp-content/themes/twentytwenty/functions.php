...
function getMenu(WP_REST_Request $request): WP_REST_Response
{
    $params = $request->get_params();

    $menu = wp_get_nav_menus($params["slug"]);
    $items = wp_get_nav_menu_items($menu[0]->term_id);
    $response = new WP_REST_Response($items, 200);
    $response->set_headers(['Cache-Control' => 'must-revalidate, no-cache, no-store, private']);
    return $response;
}

function getPost(WP_REST_Request $request): WP_REST_Response
{
    $params = $request->get_params();
    $slug = $params["slug"];
    if ($slug == null) {
...
add_action('rest_api_init', function () {
    register_rest_route('cms/v1', '/getMenu', [
        'methods' => WP_REST_Server::READABLE,
        'callback' => "getMenu",
    ]);
    register_rest_route('cms/v1', '/getPost', [
        'methods' => WP_REST_Server::READABLE,
        'callback' => "getPost",
    ]);
});
